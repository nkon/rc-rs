...
閉括弧を入力した時に対応する括弧を強調したとして、すぐに次の入力に備えなければならない。しかしカーソルを移動した時のハイライトは、さらにカーソルを移動した時まで継続される。これは括弧強調のタイマ動作（およびそのキャンセル操作）を隠蔽するための工夫でもあり、実際に使用感を損なわない小細工だ。実装上はエスケープ・シーケンスの送信とディレイの組み合わせになり、強調表示をしている間に他の作業を並行に行いづらいという課題がある。それを隠蔽するた仕様でもある。
