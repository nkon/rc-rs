...
計算の実行では無く、アプリの動作を変更したいときなどに「コマンド」が用意されている。たとえばデバッグ設定の変更や出力フォーマットの変更などだ。コマンドは実行結果が文字列となるので、`Node::Command`はそのための領域がある。使いやすさと区別のため、コマンドの引数は"()"で囲んだり","で区切ったりせずにスペース区切りとしている。
...
ユーザ定義関数では、実装の簡略化のために、引数を固定識別子で取り扱うことにした。`_1`..=`_9`という識別子を1番目〜9番目の引数として取り扱う。

関数定義は`defun`というコマンドで行われる。`defun`はコマンドなので、関数定義部分はパースされずにトークン列のまま、関数辞書の`HashMap`に登録される。

評価時には、関数の呼び出し側が与えた引数で、関数定義のトークン列中の、`_1`..=`_9`の特殊識別子を置き換えたトークン列を生成する。これをパーズして評価することで関数の値を得る。
