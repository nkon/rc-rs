...
`&str`型のリテラル文字列を`String`に変換するときは`to_owned()`の方が若干効率が良い。
...
当初、Cでの類似の実装経験から、Node構造体などをStructで表し、Node.Typeといったメンバー変数で処理を分岐するように書いていた。その後、Rustらしく、タグでの分岐をEnumの型マッチ分岐に修正した。処理漏れもなくなるし、余計な初期化や分岐も無くなった。短くて情報量が多い、なんというか「力強い」コードになる。Enumの威力を実感した。
...
"[m]"のように鉤括弧を使って、数値に単位を付加することができる。

* "10k"というように入力すれば、接尾子機能により、10*1000という数値として解釈される。
* "10[m]"と入力すれば、単位機能により10という数値と[m]という単位として解釈される。
* "10[km]"と入力すれば、(接尾子機能ではなく)単位変換機能として10*1000[m]として解釈される。
* "10[in]"と入力すれば、単位変換機能として10*0.254[m]として解釈される。
* "10[in]/1[m]" と入力すれば、評価の結果、単位が割り算されて 0.254(無単位)が出力される。

内部的には、数値ノード(`Node::Num`, `Node::FNum`, `Node::CNum`)が単位のフィールドを持つ。

単位は、パーサによって式と同様に、同じパーサを用いてASTとして構築される。

評価時に、数値はどんどん計算されていくいのだが、計算されるとき、単位はASTのまま演算される。
つまり、"2[km]\*3[s]"という式を評価すると、次のような動作となる。

1. まず、[km] => 1000*[m] というように変換してASTに挿入する。
2. 次に、数値部分は数値として演算されるが単位部分はASTのまま掛け合わせのノードが構築される。結果として"6000[m*s]"となる。
3. 式全体の評価が終わったあと、単位部分を単一の分母・分子の分数型に整理して
4. 約分する。

入れ子になったASTを、単一の分母・分子をもつ分数に変換するには、"g", "s", "m" を仮の単位とした場合、次の規則を再帰的に適用する。
...
単一の分母・分子をもつ分数になったあとは、分母、分子ごとに、各次元(長さ、重さ、、、)をキーにした`HashMap`に格納し、分母・分子で次元ごとに約分する。たとえば、"[m^3\*g^2/(g\*s)]"のような単位になった場合、分子の`HashMap`は`{"m" => 3, "g" => 2}`、分母のHashMapは`{"g" => 1, "s" => 1}`となり、約分すると、分子は`{"m" => 3, "g" => 1}`、分母は`{"s" => 1}`となる。

このときの注意として、Rustの`HashMap`のデフォルトの実装では、セキュリティを高めるために、イテレータでキーが出てくる順番が毎回ランダマイズされる。テストとして文字列化して比較する場合には、キーをソートしてから文字列化しなければ、テストが不正に失敗してしまう。

現段階では未実装機能も多い。

* [V*A]を[W]に簡略するような機能は未実装。
* 複素数の場合の単位演算は未実装。
* 加減算の場合は両辺の単位が揃っていないと行けないが、今は単に右辺の単位を採用している。
* 二乗や平方根演算での単位の換算は未実装。
